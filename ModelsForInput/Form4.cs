namespace Reference_Aids.ModelsForInput
{
    public class Form4
    {
        public int Category { get; set; } = 0;
        public int Total_4 { get; set; } = 0;
        public int Man_5 { get; set; } = 0;
        public int Woman_6 { get; set; } = 0;
        public int Child_7 { get; set; } = 0;
        public int Teenager_8 { get; set; } = 0;
        public int Anon_9 { get; set; } = 0;
        public int TotalAnalyzes_10 { get; set; } = 0;
        public int Ifa_11 { get; set; } = 0;
        public int PcrIb_12 { get; set; } = 0;
        public int Man_13 { get; set; } = 0;
        public int Woman_14 { get; set; } = 0;
        public int Child_15 { get; set; } = 0;
        public int Teenager_16 { get; set; } = 0;
        public int Row { get; set; }
    }
}
